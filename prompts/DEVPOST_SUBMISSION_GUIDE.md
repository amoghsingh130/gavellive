...
| **Tagline** (≤200 char) | A live-commerce auction platform built to scale to millions — thousands bidding on one lot at once, every sale provably correct: zero lost bids, one winner, guaranteed by Amazon Aurora DSQL. |
...
| **Track / category** | Million-scale global app (live-commerce entertainment) |
...
| **Video demo** | https://www.youtube.com/watch?v=IcCN94iFmVs (confirm Public or Unlisted, **not** Private) |
| **Image gallery** | Upload `diagrams/thumbnail.png` **first** (becomes the card thumbnail; true 3:2), then `diagrams/01-system.png`, then `diagrams/dsql-cluster.png`. |
...
GavelLive is a real-time live-commerce auction platform for high-demand collectibles — watches, classic cars, jewelry, rare books — a live-event entertainment experience where the database is the hero. It's built on Amazon Aurora DSQL for the H0 Hackathon (Hack the Zero Stack with Vercel v0 + AWS Databases).

## Inspiration

Live commerce — Whatnot, TikTok live drops, real-time bidding events — is one of entertainment's fastest-growing formats, and it's a brutal distributed-systems problem the instant it goes global: many people worldwide bidding on the same item at the same instant, money on the line, and no room for error. Two bids must never both "win." The price must only go up. The last bid before the clock hits zero must count. Most demos hand-wave this. We wanted to build a live-event experience that scales that drama to millions of concurrent bidders *and* prove — on screen — that every sale stays correct under load, using a database built for exactly this guarantee.

## What it does

GavelLive is a live-commerce experience built around timed live auctions. A buyer browses the floor, opens an auction, sees the current price and countdown updating in real time, places a bid, and is told instantly whether they're the high bidder or were outbid. Anti-snipe extends the clock when a bid lands in the final seconds. When the clock hits zero, exactly one winner is finalized from the source of truth. Under the hood, every bid runs as a serializable transaction with an optimistic-concurrency retry loop, and the architecture is designed so that same flow holds whether ten people or millions of bidders worldwide hit one lot at the same instant.

## How we built it

- Frontend: Next.js (App Router) with a hand-built UI, deployed on Vercel.
- Source of truth: Amazon Aurora DSQL — serializable, strongly consistent, optimistic concurrency control, accessed from Vercel serverless functions via short-lived IAM auth tokens (no static passwords). It is the primary database for users, auctions, and bids; we chose it because serializable strong consistency and OCC make correct concurrent bidding *provable*, not just hoped for.
...
We didn't just claim correctness — we proved it on screen. 300 concurrent bids against the live API on real Aurora DSQL:
...
The whole thing runs serverless on Vercel against a managed, IAM-authed DSQL cluster — no static credentials, no single-writer bottleneck, and the invariants hold live in front of judges.

## What we learned

- Pushing correctness down to a serializable database lets the app layer stay simple: the database, not application locks, is what guarantees exactly one winner.
- Optimistic concurrency control is the right model for bursty contention — but only if you cleanly separate retryable conflicts (SQLSTATE 40001) from terminal business rejections, or you either retry forever or fail valid bids.
- Building on a PostgreSQL subset means re-thinking assumptions (no FOREIGN KEYs) and moving referential integrity into the app, which is a fair trade for DSQL's consistency and scale guarantees.
- Short-lived IAM auth tokens make serverless-to-database access secure by default, but demand connection patterns built for ephemeral function lifecycles.

## What's next for GavelLive

- Real-time push via DynamoDB Streams → Lambda fanout over API Gateway WebSockets (replacing ~750ms polling). Amazon DynamoDB would serve hot-state + the bid-event stream for low-latency, high-fanout reads at scale.
